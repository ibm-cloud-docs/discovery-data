...
1.  Run the `all-backup-restore.sh` script.
    ...
    If you run into issues with the backup, rerun the backup command and include the `--use-job` parameter. This parameter retains any log messages that are generated by the script. You can review the logs to check for the cause of any issues.
