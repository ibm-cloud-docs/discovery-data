...
`*` Some DOC, PPT, and XSL files that use older features which are no longer supported by Microsoft can cause ingestion issues. If you encouter an issue or want to avoid issues, you can open the files in a more recent version of Microsoft Office and convert the files to the DOCX, PPTX, or XSLX formats respectively before you upload them.
