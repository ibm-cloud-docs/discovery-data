...
Currently, when you disable FAQ extraction on collections that contain HTML or TXT files, the original documents are not restored and the question-and-answer pair documents that were generated by FAQ extraction are not deleted. This limitation applies to HTML files that are added to a collection from web crawls, for example.
{: important}
