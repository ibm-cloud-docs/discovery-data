...
If you decide not to use the Plus plan and don't want to pay for it, delete the Plus plan service instance before the 30-day trial period ends.
{: important}
...
1.  Repeat this process to label and submit multiple pages.
    ...
    If the tool becomes over zealous in the application of labels, apply the `text` label to sections of standard text to correct it.
...
In this tutorial, you created a search project, a Smart Document Understanding (SDU) model, and a dictionary enrichment. You then applied a dictionary facet to the custom field that is generated by your SDU model, and used the facet to filter your query results for better answers.
