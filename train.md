...
If you're familiar with the Discovery Query Language, you know that when you submit a DQL query, you can specify which fields from the indexed documents you want the result to come from, and how much significance to give to each one. You cannot do so with natural language queries. In fact, searches triggered by a natural language query consider all of the root-level fields in your documents to have equal significance. One of the things you can teach Discovery with relevancy training is which document fields are more significant than others. When you train, Discovery builds a model that assigns different weights to term, bigram, and skip-gram matches for each of the root-level fields and balances them against matches from all of the other document fields. With enough examples, Discovery can return better answers because it knows where the best answers are typically stored.

Relevancy training cannot be used to give more weight to nested fields because nested fields are grouped together and assigned one overall score. No matter how much you train, Discovery will never give a nested field more weight than it gives to a root-level field.
{: note}
...
The training data that is used by the relevancy model includes these parts:

-   A natural language query that is representative of a query your users might submit
-   Results of the query, which are returned by the service
-   The rating you apply to the result that indicates whether the result is `relevant` or `not relevant`.
...
1.  Enter a natural language query in the **Enter a question to train** field.
    ...
    Do not include a question mark in your query. Use the same wording as your users. For example, `IBM Watson in healthcare`. Write queries that include some of the terms that are mentioned in the desired answer. Term overlap improves the initial results when the natural language query is evaluated.
1.  Click **Add+**.
1.  Click **Rate results**.
1.  After the results are displayed, assess each result, and then select **Relevant** or **Not relevant**, whichever option is appropriate given the quality of the result.
...
1.  You can continue adding queries and rating results after you have reached the threshold. Enter all of the queries that you think your users will ask.
...
The `document_retrieval_strategy` can be found under the `retrieval_details`. If you query a trained collection by using the {{site.data.keyword.discoveryshort}} Query Language, or the trained model is temporarily unavailable, the `document_retrieval_strategy` is `untrained`.
